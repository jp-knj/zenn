...
### コンテンツ戦略の組み込み
製品にコピーやコンテンツを追加する場合、定義されたスタイルガイドがあればかなりうまくいきます。スタイルとトーンに関する明確なガイドラインと、仕組みに関する一般的な質問に対する回答は、首尾一貫した製品を開発するために不可欠です。しかし、スタイルガイドには限界があります。特に、UI要素やパターン化されたコンポーネントの微妙なスタイリングに関しては。
だからこそ、デザインシステムを構築する際にコンテンツ戦略を取り入れることが重要なのです。コンテンツやマイクロコピーは、デザインシステムの構成要素のいたるところに登場する。早期に標準化する努力をしなければ、コンポーネントを実装するたびに、どのようにコピーを表示すべきかを決定することになる。明確に定義されたコンテンツ戦略は、いつ、どのようにコピーが使用されるかについてチームに明確なパラメーターを与えるため、より良いデザイン決定を促進するのに役立ちます。
土台が不安定なまま製品の規模を拡大することは避けたい。ありがたいことに、コンテンツのスタイリングとコピーに関する問題を標準化し、チーム全体のためにこれらの決定を文書化する方法を正確に決定することで、多くの問題を早期に回避することができます。
コンテンツ戦略をどのようにデザインシステムに組み込むかは、あなたのチームや製品によって異なります。そのため、あなたが含めることができるすべてのものを網羅的に説明するのではなく、よくある犯人をいくつか見て、それらが効果的に標準化され、文書化される方法を考えてみましょう。

大文字と小文字
一度探し始めると、タイトルと文の大文字と小文字の選択は、モダルのヘッダー、ボタンのコピー、テーブルの見出し、テキスト入力フィールドなど、製品内のほとんどあらゆる場所で出てくるようになります。リストは続く。
しかし、どちらか一方を選んでそれに固執すればいいという問題ではありません。どこに何を使うかを賢く選択することで、UI要素を区別し、必要なときにテキストに注意を喚起することができます。
何が違うのか？タイトルケースの場合、見出しのすべての単語を大文字にします（代名詞や「および」などの調整接続詞を除く）。文の場合は、最初の文字だけを大文字にします。つまり、こうなります：
タイトルケースフォルダとドキュメント
センテンスケースフォルダとドキュメント
これは、例えばボタンなど、ほとんどが1つの単語で構成されるものをデザインするときによく出てきます。しかし、後になって2つの単語が必要になる場合があります。2つ目の単語を大文字にするのかしないのか。事前によく考えておかないと、結局2つの間を行き来することになります。
大まかな区別はできます。タイトル・ケースはよりフォーマルで注目を集め、短い文字列をより目立たせることができる。しかし、非常に長い文字列では不格好になりがちです。センテンス・ケースは一般的に少し読みやすく、特に数単語より長いものには適しています。
私のアドバイス：テーブルの見出しやボタンなど、短い文字列が近くに連続して表示される場合は、タイトル・ケースを使いましょう。ユーザーの注意が一度にひとつの文字列に集中するような場合、特にそれが数語以上の長さになりそうな場合は、センテンスケースを使いましょう。これは、モーダルヘッダーやテキスト入力フィールドのプレースホルダーコピーでよく見られます。
このような決定事項は、スタイルガイド全体の中に入れてしまうと、わからなくなってしまうかもしれません。その代わりに、コンポーネントのビジュアルガイドラインと一緒に文書化することを検討してください。そうすることで、コンポーネントを実装する際に、そのコンポーネントを参照する人にとって、タイトルまたはセンテンス・ケースを使用するかどうかの決定が念頭に置かれるようになります。
しかし、ここで注意点があります。それは、スタイルガイドにタイトルとセンテンスの大文字と小文字の概要と、一般的に言って、それぞれをいつ使うべきかを記載することも意味があるということです。というのも、仕事を始めたばかりの段階では、すべての状況でどの選択肢を使うべきかを定義することはおそらく不可能だからです。全体的な視点を文書化することで、適切な判断を下すための適切なツールを人々に提供することができます。
一般的な語彙
削除」、「廃棄」、「撤去」はどのような場合に使うのでしょうか？"エクスポート"、"ダウンロード"、"共有 "はどうでしょうか？これらはすべて異なる意味を持っていますが、それぞれの集合の中で、矛盾にぶつかる可能性があるほど重複しています。
最悪のシナリオは、ユーザーがシナリオによって異なるアクションを求められていると思うほど、用語が混在してしまうことです。このような事態を避けるには、設計を始める前にすべての用語をマッピングしておくか、製品を監査して矛盾を特定します。
使用する用語について強い視点を持ち、用語集を作成する。互換的に使用される可能性のある用語に出くわすたびに、確固たる見解を示し、その決定を用語集にまとめましょう。これを早く実行し、習慣化すればするほど、製品設計がより明確になります。
用語集は、人々が実際に使って初めて役に立つものです。スタイル・ガイドに含めるか、デザイン・システムに組み込むかは、誰が使うかによります。
各用語がどのように使われるべきかを、関連する他の用語との曖昧さをなくすとともに明確に示すとよいでしょう。また、密接に関連する2つの用語の両方を製品で使用する必要がある場合（「export」と「download」の使い分けなど）は、区別しておくと便利です。このような場合は、それぞれの良い使い方と悪い使い方の例を挙げて、単純に並べてリストアップすると便利です。
空の状態
空の状態は、しばしば後回しにされがちだが、そうである必要はない。むしろ、具体的な目標を達成するためのコンテンツ・ガイドラインとして最適な場所なのです。空の状態は、ユーザーが今まで遭遇したことのない機能を理解するのを助けたり、ユーザーに製品の使えない部分にぶつかったことを説明したりします。
空の状態に関しては、通常、イラストが主な関心事となりますが、必ずしもそれが最も重要な部分というわけではありません。というのも、エンプティステートにアプローチする最善の方法は、ユーザーがどのようにしてブロックを解除できるかを明確にすることだからだ。
ユーザーがあなたの製品について達成しようとしていること、発見しようとしていることが何であれ、そのブロックを解除するためにコピーをどのように使用できるかを明記したガイドラインを作成する。答えるべき質問は、この機能を彼らにとって有用または関連性のあるものにするために、ユーザーは何を知る必要があるかということだ。
空の状態のためのコピー・ドキュメンテーションは、イラストのためのガイドラインのような、空の状態のための他のガイドラインと簡単に共存させることができる。空の状態におけるコピーの目標を明確にしなければ、視覚的には一貫しているが、最終的にはあまり役に立たない状態になってしまう可能性が高い。あなたのゴールはユーザーを宙ぶらりんの状態から脱出させることであり、そのための最善の方法は、あなたのメッセージングがそれを達成するための明確なガイドラインを提供することです。
スタイルとトーンのガイドラインがマイクロコピーに特に役立つことは難しい。結局のところ、非常に短い文字列を扱うことが多く、多くの場合、言語と視覚的な表現との結びつきは、コミュニケーション方法の不可欠な部分なのです。デザインシステムを使えば、一般的な動作に使用される言語についてガイダンスを提供し、当て推量を排除することができます。
ツールチップとエラーメッセージは、ここでの主要なターゲットです。どちらも、製品の声をどの程度反映させるかを決める必要があります。エラーメッセージでは申し訳なさそうにしたいかもしれない。あるいは、起こっている事実をできるだけ明確に述べることもできる。どちらを選ぶかは、製品のフィーリングによります。
いずれにせよ、メッセージの具体的な要素を定義するのに役立ちます。どのような代名詞を使い、どのようにユーザーに語りかけるかを示してください。製品の背後にある声を指すのに "we "を使いたいのか、それとも完全に避けるべきなのか。
確認ダイアログにはいくつかの異なる課題があり、"OK "と "OK "のどちらを使うかを明確にするだけではありません。ここでのガイドラインは、あなたの製品でどのように進めるかについて、ユーザーに極めて明確な選択肢を与えることがすべてです。
これを行う 1 つの方法は、モーダルヘッダーと確認およびキャンセルアクションのボタンコピーの関係を定義することです。これら両方の書き方を明確にするためにできることは何でも、デザイナーが製品全体に一貫性を持たせるのに役立ちます。たとえば、ヘッダーの先頭の動詞を確認ボタンに合わせることで、2つが互いに反映され、ユーザーにアクションを明確に伝えることができます。
あなたの製品のマイクロコピーを一人の人間が書くことはまずないでしょうから、乱暴なメッセージングになってしまいがちです。時間をかけて、それぞれのタイプのメッセージの形を整理すれば、製品に一貫性を持たせることができます。
ローカリゼーション
一番最後に紹介するのは、みんなが大好きなトピック、ローカライゼーションです！国際化の努力は、日付のフォーマット、通貨、気温、住所のフォーマットなど、さまざまな違いを考慮する必要がありますが、UI要素にも実際に影響を与える領域があります。ローカライゼーションを後回しにすると、後々難解な問題を抱えることになります。
まず考慮すべきは文字列の長さだ。ドイツ語だけが原因ではありません。他言語に翻訳することで、文字列の長さが長くなり、設計が崩れてしまうケースはたくさんあります。そのため、UI要素に必要なときにテキストを縮小する方法があることを確認する必要があります。これは、ナビゲーション・バーや製品のトラフィックの多い部分で特に重要です。
ローカライズする言語ごとにイラストを作り直す必要が出てくるので、イラストの中のテキストは避けましょう。また、他の文化圏で混乱を招きかねない視覚的なメタファーにも気を配りましょう。
また、ローカライズされたコンテンツの語順が同じであることを当てにすることはできないことにも留意してください。例えば、文章を完成させるために複数のテキスト入力フィールドがある場合、コンテンツが翻訳されたときにUIの混乱を招くことになります。
ローカライゼーションが難しいのは、単純に考えてもみなかった分野の変更に簡単にぶち当たる可能性があるからだ。私は、デザインシステムやスタイルガイドの中に、ローカライズを考慮した別のセクションを作ることをお勧めします。こうすることで、デザイナーやコンテンツ戦略担当者は、すべてのガイドラインを一箇所で見ることができ、全体的に考えることができます。各ガイドラインをセクションに分け、ローカライゼーション戦略の不備がデザインをどのように壊してしまうかの例を示すようにしましょう。最良のアプローチは、このことを常に念頭に置き、ローカライゼーションの変更がどのような意思決定に影響を与えるかを考慮することに、デザインシステムの全員に慣れてもらうことです。
