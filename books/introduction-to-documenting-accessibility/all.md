...
### デザインコミュニティとモーション
デザインコミュニティは、デザインとコードの両方でコンポーネントを文書化し体系化することに厳密さを適用し始めているが、モーションはしばしば議論から取り残されている。しかし、デザインにモーションを使用することの要件は、デザインシステムのガイドラインに完全に合致しています：再現性、時間の節約、UXの一貫性です。
...
- ユーザーを教育する：モーションは、追加クリックなどのアクションを必要とせずに、文脈や情報を提供することができます。
- ユーザーを集中させる：モーションはユーザーの注意を引き、重要なデータや重要なUI要素を強調することができます。
...
### ステップ1 - 監査から始める
何か新しいものを作る前に、現在あるものを見てみましょう。UIパターンの監査を行うのと同じように、モーションパターンについても同じことができます。これには時間がかかりますが、デザインシステムに追加すべき同じ要素の繰り返し使用や、普遍的な解決策を提供しない可能性の高い単発的な使用がどこにあるかを示すことができます。
...
### モーションにおける一貫性の重視
使用している書体やフォントサイズの数を減らすために監査を行うのと同じように、モーションについても同様の戦略を採用したい。モーションシステム全体の一貫性を重視することで、ユーザーにとって快適で予測可能な環境を作ることができます。
...
1. **マテリアルデザイン**：  
   標準化の試みとして、マテリアルデザインはモーションの3つの原則を定義。チームは、これらの原則に基づき、階層、フィードバック、ステータス、キャラクターアニメーションという4つの用途を決定。  
   *出典：マテリアルデザイン*

2. **IBM**：  
   IBMはモーションを「表現的」または「生産的」と考えている。これは開閉状態に関連するものの、他の場面でも応用できる。  
   *出典：IBM*

3. **Salesforce**：  
   Salesforceは、ブランドのパーソナリティをアニメーションに統合する際のアプローチを示している。ブランディングされたモーションはコンテクストに応じて適用される。  
   *ソース：Salesforce Lightning*

#### モーションの原則の分類
これらの例から、モーションの原則は主に「喜びとしてのモーション」と「ユーザビリティとしてのモーション」の2つのカテゴリに分類されます。どの粒度のレベルでこれを行うかはチーム次第です。適切な指針があれば、創造性と一貫性を両立させることが可能です。
...
#### エフェクトの体系化
エフェクトを整理する方法の1つは、それらを原則にマッピングすることです。このマッピングは、プリンシプル、コンポーネント、エフェクト、デュレーション、イージーの関係を明らかにします。
...
タイポグラフィでの`h1`, `h2`, `h3`, `h4`, `h5`のように、モーションのスケールに`t1`, `t2`, `t3`, `t4`, `t5`のタグを使用することができます。また、`Extra Fast`, `Fast`, `Normal`, `Slow`, `Extra Slow`のような説明的な名前も選択可能です。
...
- **オブジェクトベース**:
  ...
  - ページ遷移: 500MS - 700MS
- **サイズと距離**:
  - ビューポートの10%移動ごとに100MS
- **アニメーションの複雑さ**:
  - 2～5個のオブジェクトが連続してアニメーションするトランジション: 300～400MS
  - 6～10個のオブジェクトがアニメーションするトランジション: 500～700MS
...
採用のためには、デザイナーはプレゼンテーション・ファイルをプロダクション・コードに合わせて翻訳する必要があります。動きを文書化する最も簡単な方法は、開始状態と終了状態をマッピングすることです。これが、状態Aと状態Bの間の変化を補間するキーフレーム作成の前提です。

ストーリーボード、スケッチ、またはMP4ビデオは、開発者がプロトタイプを作るために必要なすべてを提供することができます。しかし、モーションシステムを拡張するためには、トランスレーションを含めることが重要です。

### モーションコンセプトの文書化方法

モーションコンセプトを文書化するには、ダイアグラムとテキストという2つの効果的なテクニックがある。

#### テクニック1：ダイアグラム仕様

お好きなソフトウェアを使用して、通常、設定パネルを開き、ダイアグラムチャート（ガントチャートのようなもの）をスクリーンショットまたは手動で文書化することができます。
...
この2番目のアプローチはコードに似ていますが、デザイナーはコードを話すことを恐れる必要はありません。

- 不透明度：パーセントで定義：0% - 100%
- スケール：単位またはパーセントで定義：1px、2000px、0～100
- 標高：Z軸：0、1、2など
- 移動：x座標とy座標（画面の幅と高さに対する割合でも可）
- 色：HEX値

このアプローチの利点は、テキストが簡単に編集でき、これらのデータ・ポイントをより簡単にプロダクション・コードに変換できることです。

After Effectsには、キーフレームのプロパティをテキストファイルに変換するInspector Spacetimeというプラグインがあります。

物理、加速度計、パーティクルシステム、3Dモデルなどの高度なモーションコンセプトの場合、1:1の変換を見つけるには、より手作業が必要になります。このプロセスでは何でもそうですが、コードパラメータを理解するためにディスカッションを行うことで、正しい設計戦略を知ることができます。
...
一見すると、デザインにおけるモーションは複雑すぎて、デザインシステムに適した構成要素に分解できないように思えるかもしれません。しかし、これらのテクニックを使うことで、モーション解析を排除し、製品開発においてより迅速で革新的な方法を可能にするシステムを構築することができます。

私たちのデザインシステムにモーションを取り入れることは、より良いコードベース、プラットフォーム全体のUXの一貫性、そしてユーザーにとってより大きな喜びの瞬間を意味します。
