...
MySpace は一般的に使われていない水平ナビゲーションを使用している出典： myspace
検索バー、ログイン画面、ナビゲーションメニューなど、どのサイトにも繰り返し使用される要素には、確立された標準があります。新たに発明するのではなく、それらの標準を再利用しましょう。
Mercado Livre は最新の e コマースのプラクティスに従っている出典： Mercado Livre
6. シンプルに保つ
   心理学者のポール・フィッツが 1954 年に発表した研究で提示した内容に従うなら、ボタンは大きく、アクセシビリティに配慮されたものであるべきです。ユーザーが決断しやすい状況をつくるのはデザイナーの責任です。
シンプルにするためにアクセシビリティを犠牲にしない。コントラスト不足は、認知負荷を高める。
シンプルにするためにアクセシビリティを犠牲にしない。コントラスト不足は、認知負荷を高める出典： Typeset Design
...
Mural. Mural は、ホワイトスペースと高いコントラストを使用して、ページ上の最も重要なコンテンツを明確に示す
...
文章の構造と内容はおおよそ整っているのですが、言い回しや表現に関しては少し矯正や調整が必要と感じました。以下、修正を施した文章です。
...
マイクロコピー
簡潔で一貫した言語と視覚的表現は、コミュニケーションの鍵です。デザインシステムを利用すれば、常用される言語に対するガイドラインを提供し、不確かさを排除できます。ツールチップやエラーメッセージは、ここでの重要なポイントです。これらのメッセージにおいて、製品のトーンやキャラクターをどれだけ反映させるかを決定する必要があります。
確認ダイアログでは、ボタンのラベルだけでなく、その背後にある意味やメッセージをユーザーに明確に伝えることが必要です。こうしたガイドラインを整理し、デザイナーが製品全体を一貫してデザインするためのサポートをすることが重要です。
ローカライゼーション
