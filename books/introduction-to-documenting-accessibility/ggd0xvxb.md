...
アクセシビリティとコーディングの標準から始めます。コンポーネントの HTML 構造とアクセシビリティの配慮を前もって確立しておくと、後々不必要な悩みを省くことができます。
デザイナーとして、良い技術的理解を得るのを助け、制限を尊重するようになりました。コンポーネントのベストプラクティスを調査する際の 4 つの出発点は以下の通りです

#### WCAG
アクセシビリティの業界全体の基準は WCAG、Web コンテンツアクセシビリティガイドラインです。ほとんどのチームは WCAG AA の基準を満たすことを目指します。ほとんどのユーザーがあなたのプロダクトを利用できることを意味します。

WCAG は非常に多岐にわたっており、何を探しているかを知らない限り、どこから始めるべきかがわからないことが多いです！幸いなことに、Web Accessibility Initiative（WAI）はアクセシビリティのための執筆、デザイン、開発方法に関する簡略化されたガイドラインを提供しています。コンポーネント固有のガイドラインについては、WAI の Web アクセシビリティチュートリアルや ARIA Authoring Practices Guide のパターンセクションを参照してください。
...
コンポーネントの研究において私の 2 番目の参考資料は MDN web docs です。調査しているコンポーネントを検索すると、セマンティックな HTML 構造、属性、および関連するアクセシビリティの懸念事項が表示されます。

#### 情報
Vitaly Friedman の「Complete Guide To Accessible Front-End Components」は数百の有用なリンクを集約しています。もし、あなたが取り組んでいるコンポーネントが Heydon Pickering の「Inclusive Components」に掲載されていれば、やった！既にその作業はあなたのために完了しています！

#### 他のデザインシステム

私はまた、他のデザインシステムもチェックします。Component Gallery や Design System's Docs Search は、調査しているコンポーネントを検索できるデザインシステムの集約サイトです。FigJam や Miro のようなホワイトボードツールでデザインシステムのドキュメントをペーストアップすると、比較に役立ちます。

ここで注意喚起の言葉を 1 つ - デザインシステムに含まれているからといって、それがアクセシブルである、またはあなたの製品のユーザーに適しているとは限りません。注意を払い、常に調査を行ってください。
...
ブレッドクラムコンポーネントを使って、プロセスの各段階を示します。多くの方がブレッドクラムを知っているでしょう。それは、複雑なウェブサイトをナビゲートするのを助けるリンクの水平リストです。


WCAG 2.4.8:位置に関連しています
順序付きリストを使用して構造化されたリンクのセットで構成されています
nav 要素でラップします。これにより、スクリーンリーダーなどの支援ツールのユーザーがブレッドクラムトレイルをナビゲーションのランドマークとして識別できるようになります。このランドマークをラベル付けするには aria-label="Breadcrumbs"を使用します
項目間のセパレータは非対話型です。aria-hidden を使用してスクリーンリーダーがそれらをアナウンスするのを防ぐために、それらを装飾要素にできます
現在のページのリンクには aria-current を追加します
アクセシビリティを始めたばかりの方は、これらの調査結果が少し難解に感じるかもしれませんね。大丈夫です、一緒に仕様を詳しく見ていく中で、これらのポイントが何を意味するのかを解説していきます。

https://www.w3.org/WAI/WCAG21/Techniques/general/G65.html
#### コンポーネントの構造
ある程度の調査を行い、基礎となる HTML のアイディアを持っている場合、この知識を使用してコンポーネントの構造をマッピングできます。コンポーネントは、アイコン、ボタン、リンク、リストなどのより小さな構成要素で構成される可能性があります。構造の図をスケッチすることで、チームとの思考を共有し、複雑なコンポーネントの場合には、要素やラッパーの命名規則について合意するのに役立ちます。
...
項目 | 名前 | 説明 | コンポーネント
--- | --- | --- | ---
1 | ブレッドクラム | 'ブレッドクラムの項目'とセパレータを組み合わせる | 順序付きリスト
2 | ブレッドクラム項目 | 親レベルのページへのリンク | リンク
3 | セパレータ | ブレッドクラム項目の後に表示される視覚的なセパレータ | アイコン

#### すべてのオプションを含む
コンポーネントに必要なさまざまな使用ケースのオプションを定義する必要があります。コンポーネントの概要が利用するチームから来た場合、すべての要件について話し合ってください。すべての機能を最初から構築する必要はありませんが、最初からスコープを理解することで、将来の拡張を考慮した合理的な選択ができます。
...
#### オプション
コンポーネントには、次の 2 つのオプションを含めました：

- サイズ：システム内の他のコンポーネントとの視覚的な一貫性を持たせるための SM、MD、LG
- カスタマイズ可能なセパレータ：消費者はデフォルトのアイコンを上書きするか、アイコンの代わりに別のコンポーネントを使用できます

#### 状態を考慮する
コンポーネントがインタラクティブな場合、状態を考慮する必要があります。バリデーションをサポートするチェックボックスのようなフォームコンポーネントの場合、予想よりも多くの状態が存在するかもしれません！十分な色のコントラストとそれらの視覚的な違いを確認するために、状態を視覚的に仕様化します。WebAim は、不確かな場合の色のコントラストのアクセシビリティに関する役立つガイドを提供しています。

ボタンやフォームコンポーネントなどの HTML 要素では、状態のスタイリングはブラウザのデフォルトのスタイルになります。現在の焦点がある要素を強調表示する明確な焦点インジケータは、アクセシビリティにとって非常に重要です。これにより、視覚を持つユーザーはページ上の位置を確認し、ナビゲートできます。焦点インジケータのアクセシビリティに関する詳細な情報は、Sara Soueidan のガイドを参照してください。
...
- 他のウェブサイトでこのコンポーネントがどのように動作するかを見て、適切な動作を決定するための要件を再評価してください。

#### 動作
ブレッドクラムの動作はシンプルに保ちました：

- デフォルトでは、現在のページのブレッドクラム項目は非インタラクティブとして表示されます
- 最後のトレイリングセパレータはデフォルトで非表示です。必要に応じて表示に設定できます

キーボードの操作とフォーカスの順序
キーボードの操作は Web のアクセシビリティにとって不可欠です。これは、視覚障害者、身体障害者、そして多くの障害を持たない人々のために役立ちます。多くの人々はキーボードでナビゲートする方が効率的だと感じています。

コンポーネント内のすべてのインタラクティブな要素 — ボタン、リンク、コントロールなど — はキーボードのみでアクセス可能でなければなりません。Tab キーは通常、各インタラクティブな要素間を移動するために使用され、上下キーはスクロールのために、Space キーまたは Enter キーはインタラクションのために使用されます。
...
**ARIA**
Accessible Rich Internet Application、または ARIA は、スクリーンリーダーなどの支援技術を使用する人々にとって HTML 要素をよりアクセスしやすくするための属性のセットです。ARIA は、HTML が支援技術に情報を公開しない場面で役立ちます。ARIA と HTML の使用時期については、web.dev の ARIA 入門を参照してください。
...
**Propsとオーバーライド**
仕様を設定したら、定義したオプションと動作をサポートするために必要な props とオーバーライドについて考えます。私はユーザーの立場になってコンポーネントの機能をステップバイステップで進めるのが好きです。私たちのブレッドクラムの例では、サイズを設定し、アイコンを表示/非表示にするための prop が必要です。スタイリングとパディングを変更するためのオーバーライドも役立つでしょう。NewsKit の完全なブレッドクラム API テーブルはこちらで確認できます。
...
デザインシステムのアクセシビリティに関してどの段階にいても、ここでいくつかの洞察を得られたことを願っています。各チームのセットアップは異なり、プロセスを共有することでお互いに学び取ることができます。NewsKit のコンポ

ーネントの作成はチームの取り組みであり、このリサーチの主要な作業は、デザイナーの Mike Messina と Valeriia Bielozorova、エンジニアの Stoyan Delev と Marco Vanali によって行われました。
