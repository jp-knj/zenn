## Accessibility Conformance Testing テスト設計について

アクセシビリティを実現するためのガイドラインが WCAG として存在しますが、それをどのようにテストすればよいかは具体的には書かれていません。

ACT ルールは、「アクセシビリティ適合性試験（Accessibility Conformance Testing）ルール」の略です。その名の通りアクセシビリティの適合性を試験するためのルールを提供しています。WCAG や他のアクセシビリティガイダンスで明文化されていないエッジケースの取り扱いに重点を置いています。

Web ページのアクセシビリティを確認するためのツールは複数存在します。ツールを使うことで、手動での確認に比べて効率的に問題を見つけることができるようになルでしょう。

しかし、ツールのみに頼ったアクセシビリティ検証は、すべての問題をカバーしきれないということを理解しておきましょう。
WCAG の基準には、ツールだけで評価が難しいものが多く存在します。例として、画像の代替テキスト（alt 属性）が挙げられます。このテキストは、画像の内容を的確に伝えるものでなければなりませんが、ツールだけでは内容が適切であるかの判断はできません。
文脈に応じた評価：同じ画像でも、ページの文脈によっては異なる代替テキストを持つことが適切な場合があります。これもツールだけでは判断が難しいです。

ACT ルールは、そのテスト方法を手動テスト、自動テストにテストの標準を規則的にしてくれます。

テストツールのスコアで 100 点を取ったとしても、それは完璧な Web ページであるとは限りません。

## ACTルールとは
WCAG や WAI-ARIA として提供されています。WCAG や WAI-ARIA などの基準やガイドラインが示す「適合性」をどのようにテストすれば良いかが疑問になります。そこで役立つのが、ACT ルールです。
...
### ACTルールの目的
- 情報提供の役割： ACT ルールは、あくまで情報提供のツールとして機能します。それ自体が WCAG や他のアクセシビリティ基準の適合性を保証するものではありません。真の適合性の判断は、WCAG の成功基準に基づいて行われます
- 部分的なチェックの実施： ACT ルールは、アクセシビリティ要件、例えば WCAG 2 の成功基準をチェックします。これにより、複雑な Web ページでも簡単に一部を検証できます
- 失敗の特定： ACT ルールの主な目的は、アクセシビリティ要件の不適合な部分、すなわち「失敗」を特定することです。これにより、開発者は具体的な修正ポイントを把握しやすくなります
