...
Preactの開発者、Jason Millerが示した「Islands Architecture（アイランズ・アーキテクチャ）」という概念です。これは、技術的にはごくシンプルなものでした。ページ全体をJavaScriptで制御するのではなく、まずは静的なHTMLを描き出し、インタラクティブ性が本当に必要な部分だけをIsland（島）として動的に扱うという構造です。
...
### Astro：静的HTMLを基点に「島」で局所ハイドレーション

これらのフレームワークと比較すると、Astroの立ち位置がより鮮明になります。初期JavaScriptは0KB。完全に静的なページとして配信され、`client:*`ディレクティブで明示的に指定した部分だけがインタラクティブな「島」となります。

状態共有は島ごとに独立しており、島間でリアルタイムに状態を同期する場合は、開発者が明示的に外部ストアを組み込む必要があります。これは制約でもありますが、同時に「不要なJavaScriptを送らない」という原則を徹底した結果でもあります。
...
第1章と第2章の内容の違いを踏まえて、橋渡し部分を改善いたします。
...
これらの思想は、美しい理念として語られるだけでは意味がありません。実際のコードとして、具体的な実装として形にされて初めて、その価値を発揮します。

では、Astroはこの思想をどのようにコードで実現しているのでしょうか。静的なHTMLを生成し、必要な部分だけを「島」として切り出す。この一見シンプルな仕組みの裏側には、精緻に設計されたコンパイラが存在します。
