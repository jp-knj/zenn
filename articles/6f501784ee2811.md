...
この擬似的なSPA化において、Astroが提供する機能が[transition:persist](https://docs.astro.build/en/guides/view-transitions/#transitionpersist)です。
...
スワップ直前、対象のDOMノードを現在のツリーから切断し、メモリ上に退避させます。この時点でノードはドキュメントから外れますが、JavaScriptインスタンスとしては生きており、内部状態を保持し続けます。そして、新しいHTMLが挿入された後、対応するプレースホルダー部分を、退避させておいた「生きたDOMノード」で置き換えます。

これはReactによる差分検知とは根本的に異なります。Reactが「状態（PropsやState）を維持してビューを再描画」するのに対し、Astroは「物理的な要素そのものを移動」させます。これにより、フレームワーク固有の状態管理に依存せず、ブラウザネイティブの状態を維持できる点は、AstroのDOM駆動思想を象徴しています。
...
1.  サーバーでレンダリングされたコンポーネントツリーの差分データ（Payload）がクライアントに流れてきます。
2.  クライアント上のReactランタイムがこれをデシリアライズします。
3.  既存のFiber Tree（Reactの仮想DOMツリー）に対して、新しいサーバーデータをマージします。

ここでは物理的なDOMの総入れ替えではなく、メモリ上のツリー構造の論理的なマージが行われます。これにより、Reactが管理するState（動画の再生状態やフォームの入力値など）が維持されます。
...
1.  ナビゲーション開始時、Reactは `isPending` 状態になり、バックグラウンド（メモリ上）で次の画面のレンダリングを開始します。
2.  データ取得が完了し、次の画面の描画準備が整うまで、現在の画面を一切変更せず維持します。
...
ここがNext.js (React) の実装において難解であり、かつ凄みを感じる点です。ブラウザの `document.startViewTransition(callback)` は、`callback` 内でDOMが同期的に更新されることを要求します。しかし、ReactのState更新は基本的には非同期バッチ処理です。

そこでReactは以下のような挙動をとります。

1.  Reactは準備が整った新しいツリーをDOMに反映する際、並列処理の「優先度付き更新」を一時停止します。
...
非同期で動くReactの世界と、同期を求めるブラウザAPIの世界を接続するためのブリッジ処理がランタイム深部に組み込まれているのです。
...
sequenceDiagram
    ...
    rect rgb(255, 245, 238)
        ...
        React->>React: 🔄 FlushSync で強制同期する<br>メモリ上のツリーを一気にDOMへ適用
        React-->>Browser: DOM更新完了
        deactivate React

        Browser->>Browser: 📷 New Snapshot 撮影
    end
...
これは、View Transition APIとの同期時、Reactは計算済みの全てのDOM変更（コミットフェーズ）を単一のタスクとして実行することを意味します。巨大なコンポーネントツリーの変更を伴うとき、このDOM反映処理が完了するまでメインスレッドを占有します。リッチなアプリケーションでは、この一瞬のフリーズがユーザー体験に影響する可能性すらありますが、そこまでしてもReactは「状態の一貫性」を優先していると言えます。
...
Astroのアプローチは、将来的なウェブ標準を見据えたPolyfillに近いと言えます。DOMの置換と要素の永続化による実装は、ブラウザがCross-Document View Transitions (Level 2) を完全にサポートした暁には、`<ClientRouter />` を削除しても動作することを目指した、ウェブ標準に寄り添う設計です。「今は泥臭いことをしてでも、未来の標準へ橋渡しをする」という気概を感じます。

対してNext.jsは、View TransitionをReactエコシステムの一部として取り込んでいます。SuspenseやServer Actions、そしてConcurrent Renderingと協調させることで、コンポーネント単位での緻密な状態管理とリッチな体験を追求しています。これはウェブ標準をあくまで「素材」として使い、ブラウザの限界を超えた独自のアプリケーションランタイムを構築しようとします。
...
画面上のアニメーションの派手さだけでなく、その背後でDOMがどのように扱われ、状態がどのように管理されているかを理解することで、プロジェクトの要件に適したフレームワークが見えてきます。
