...
## 代表的なdot notationの例
例えば、React Contextを使った場合、`ThemeContext` というトップレベルコンポーネントがあり、それに関連する `Provider` というサブコンポーネントが `ThemeContext.Provider` のようにdot表記で参照されるのです。
```jsx
...
dot notation で実装をすると、RSCの文脈で問題が発生します。以下のissueで議論されています。
