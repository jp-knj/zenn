...
## はじめに
Web開発者やライブラリ開発者にとっては、レガシーなコードよりもモダンなコードを書きたいはずです。ですが、ライブラリやツールの公開によって多くの依存関係を抱え、Webサイトが壊れるリスクを負うこともあります。そのため、ES6+ を古いブラウザでも実行可能な JavaScript である ES5 にトランスパイル[^1]しなければならない時代がありました。IE 11 をサポートするためです。ES5 は、Web開発者にとっては対応必須でした。
