...
従来のAstro（SSG）は、ビルド時にすべてのHTMLを生成して終わりでした。しかし、動的要素が一つあるだけで、ページ全体をSSR（サーバーサイドレンダリング）にする必要があり、キャッシュ効率が落ちていました。
...
JavaScript　bundleを増やさずに、クラス名の操作や開閉などのDOM操作の延長として使えます。コンポーネントの志向と実DOM操作のちょうど良い間の使い心地です。
