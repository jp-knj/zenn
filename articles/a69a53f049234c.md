...
ですが、**State Reducerパターン** は`Usageコンポーネント`から新規ユースケースの`reducer`を定義して、`useCounter()` に引数として渡しています。
これをすることで、既存の機能や`useCounter()`に影響を与えずに機能を追加することができました。
```tsx:Usage.tsx
...
まず、Compoundパターンを使用せずに`Exampleコンポーネント`を実装してみます。
```jsx
function Example() {
  const { count, handleDecrementClick, handleIncrementClick } = useCounter({
